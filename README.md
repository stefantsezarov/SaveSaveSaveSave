...
"RiskPass helps normal people and institutions understand and manage crypto risk without needing to be experts."
1. Executive Summary
...
RiskPass answers it. Give it an address — a wallet or a token — and it checks that address against known security indicators, then returns a plain verdict: PASS, CAUTION, FAIL, or INSUFFICIENT DATA. Not a price prediction. Not financial advice. A risk read, in seconds, before you act.
...
Most tools are either a bare price chart (too simple to catch anything) or an institutional dashboard (too complex, too expensive). RiskPass sits in between.
...
RiskPass never gives financial advice. It shows the risk picture; the decision stays yours.

4. How RiskPass Works (User Perspective)
Getting started: no account needed. Paste an address, pick a chain, get an answer.

Connecting wallets: RiskPass only ever reads public, on-chain and provider-side data. It never asks for a private key, seed phrase, or withdrawal permission — no version of it needs those. Future wallet-connection convenience features will follow the same rule: read-only, always.
...
RiskPass updates regularly as scam patterns and chains evolve, tested against a standing set of adversarial cases before every release.
...
Chain adapters exist because chains work differently — an EVM contract's "mint function" and a Solana token's "mint authority" are related but distinct mechanisms, checked through different data. Each adapter validates that chain's address format, fetches data via a read-only API call, and translates the response into RiskPass's internal format. Adding a chain means writing a new adapter, not touching the decision logic.

Data sources: RiskPass aggregates from established providers rather than building its own intelligence from scratch — currently GoPlus Security's public APIs, covering EVM chains and Solana. The architecture supports more than one provider per chain over time, so no single source becomes the sole "truth."
...
Before you interact with a token or address, RiskPass checks known red flags: can the contract block selling, can the owner alter balances, is the address linked to known scams. Some findings are decisive alone — a confirmed honeypot means FAIL regardless of anything else. Others accumulate toward CAUTION.
...
Long-term: RiskPass as a simple risk layer other tools build on — wallets, tax software, portfolio trackers integrating checks rather than building their own.
...
Maya, a newcomer, pastes a friend-recommended token's contract into RiskPass before connecting her wallet. FAIL — a confirmed honeypot. She never connects. No dramatic story, just a loss that didn't happen.
...
An analyst at a small fund uses RiskPass as an independent second opinion during due diligence. When it disagrees with the fund's internal model, that disagreement itself prompts a closer look before capital moves.

10. Limitations and Responsible Use
RiskPass does not guarantee profits, predict prices, replace professional advice, or catch every possible risk — detection is built on known patterns, and those have limits.

RiskPass does provide an honest read on known indicators, surface exposures a user might miss, and support better-informed decisions — the decision itself always stays with the user.
...
Private key
A secret that proves ownership and moves funds. Never share it — not with RiskPass, not with anyone.
Stablecoin
...
API key
A credential letting a service make requests on your behalf. RiskPass only ever uses read-only ones.
Read-only access
...
12. Closing: The RiskPass Promise
...
If you use RiskPass, disagree with a verdict, or find something it missed — that's not a complaint, it's exactly what makes the next version better.
